...
const ReportBtns = () => {
    ...
    return (
        <div className='flex flex-wrap justify-start items-center gap-[1.2vw] mb-[3vh] px-[2vw] max-w-full'>
            {allowedButtons.map(btn => (
                <button
                    key={btn.id}
                    onClick={handleClick}
                    id={btn.id}
                    className='bg-[#364cbb] text-white font-semibold px-[1.4vw] py-[0.65vw] mb-[1vh] rounded-[1vw] border-none cursor-pointer whitespace-nowrap text-[0.9vw] transition-all duration-200 hover:bg-[#2a3c9e] hover:-translate-y-0.5 shadow-md'
                    style={{
