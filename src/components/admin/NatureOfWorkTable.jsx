...
const NatureOfWorkTable = () => {
    ...
    // Fetch nature of works on component mount
    useEffect(() => {
        fetchNatureOfWorks();
    }, []);
    ...
    const fetchNatureOfWorks = async () => {
        ...
        try {
            ...
            const response = await axios.get(natureOfWorks, {
                headers: { Authorization: `Bearer ${token}` }
            });
            console.log("Fetched nature of works:", response.data);
            setWorkData(response.data);
            setFilteredData(response.data);
        } catch (error) {
            console.error("Error fetching nature of works:", error);
            toast.error('Failed to fetch nature of works');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    useEffect(() => {
        const filtered = !searchTerm 
            ...
            : workData.filter(work =>
                Object.values(work).join(' ').toLowerCase().includes(searchTerm.toLowerCase())
            );
    ...
    const handleAddSubmit = async (e) => {
        ...
        try {
            const token = Cookies.get("token");
            await axios.post(natureOfWorks, newWork, {
                headers: {
            ...
            setNewWork({ natureOfWork: '' });
            toast.success(`Nature of Work "${newWork.natureOfWork}" added successfully!`);
        } catch (err) {
            console.error("Add error:", err);
            toast.error(err.response?.data?.error || 'Failed to add nature of work');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleEditClick = (work) => {
        if (editingRow === work._id) {
            const editedFieldsForRow = editedValues[work._id];
            ...
            const token = Cookies.get("token");
            axios.put(`${natureOfWorks}/${work._id}`, editedFieldsForRow, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
            ...
            .catch(error => {
                ...
                toast.error(error.response?.data?.error || 'Failed to update nature of work');
            })
            .finally(() => {
                setEditingRow(null);
                setEditedValues(prev => {
                    const newValues = { ...prev };
                    delete newValues[work._id];
                    return newValues;
                });
            });
        } else {
            setEditingRow(work._id);
            setEditedValues(prev => ({
                ...prev,
                [work._id]: {}
            }));
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            const token = Cookies.get("token");
            await axios.delete(`${natureOfWorks}/${workToDelete._id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            fetchNatureOfWorks();
            setShowDeleteModal(false);
            setWorkToDelete(null);
            toast.success(`Nature of Work "${workToDelete.natureOfWork}" deleted successfully!`);
        } catch (err) {
            console.error("Delete error:", err);
            toast.error(err.response?.data?.error || 'Failed to delete nature of work');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const renderCell = (work, column) => {
        ...
        const editedValue = editedValues[work._id]?.[column.field];
        ...
        if (isEditing) {
            return (
                <input
                    ...
                    onChange={(e) => handleCellEdit(column.field, e.target.value, work._id)}
        ...
        <form onSubmit={handleAddSubmit} className="space-y-4">
            ...
            <div className="flex justify-end space-x-3 mt-6">
                ...
                <button
                    ...
                    disabled={isLoading}
                >
                    {isLoading ? 'Adding...' : 'Add Nature of Work'}
                </button>
            </div>
        </form>
    );
            ...
            <div className="p-4 border-b border-gray-200">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                        <input
                            type="text"
                            placeholder="Search nature of works..."
                            value={searchTerm}
                                ...
                                <td className="sticky right-0 z-30 px-4 py-3 text-sm text-gray-900 bg-white">
                                    ...
                                    <div className="relative z-[31] flex justify-center space-x-2">
                                        <button
                                            ...
                                            className={`${editingRow === work._id ? 'text-green-600 hover:text-green-800' : 'text-blue-600 hover:text-blue-800 cursor-pointer'}`}
                                        >
                                            {editingRow === work._id ? (
                ...
                <div className="fixed inset-0 bg-black/25 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg p-4 w-full max-w-md">
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-lg font-bold">Add New Nature of Work</h2>
                    ...
                    <div className="bg-white rounded-lg p-6 w-full max-w-md">
                        <div className="text-center">
                            ...
                            <p className="text-gray-600 mb-6">
                                Are you sure you want to delete Nature of Work "{workToDelete?.name}"? This action cannot be undone.
                            </p>
...
export default NatureOfWorkTable;
